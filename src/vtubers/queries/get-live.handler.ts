...
export class LiveVTubersHandler implements IQueryHandler<LiveVTubersQuery, VTuber[]> {
    constructor(
    ...
    async execute(query: LiveVTubersQuery): Promise<VTuber[]> {
        ...
        for (const vtuber of allVtubers) {
            const ids = [];
            if (query.platforms.includes("youtube") && vtuber.getYoutubeId()) ids.push(vtuber.getYoutubeId());
            if (query.platforms.includes("twitch") && vtuber.getTwitchId()) ids.push(vtuber.getTwitchId());

            const stream = await this.streamRepository.findOneByQuery({
                channelId: {$in: ids}, status: VideoStatusEnum.Live
            }).catch(() => {/* ignore NotFound exceptions for streams; no stream = offline */});
