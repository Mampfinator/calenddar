...
export class YouTubeScraper {
    ...
    public async fetchPosts(channelId: string): Promise<Map<string, CommunityPost>>  {
        ...
        const postFromRaw = (rawPost: Record<string, any>): CommunityPost => {
            ...
            if (backstageAttachment) {
                ...
                switch(type) {
                    case "image":
                        ...
                        attachment.images = images.map(image => last<any>(image.thumbnails).url).map(url => `${url.split("=")[0]}=s1080`);
                        break;
                    case "poll":
        ...
        return new Map<string, CommunityPost>(rawPosts.map(rawPost => {
            const post = postFromRaw(rawPost);
            return [post.id, post];
        }));
    }
}
