...
export class TwitchEventSubGuard implements CanActivate {
    ...
    async canActivate(context: ExecutionContext): Promise<boolean> {
        ...
        if (
            !timestamp ||
            Date.now() - Date.parse(timestamp).valueOf() > ms('10m')
        ) {
            const { address } = req.socket.address() as AddressInfo;
            this.logger.log(
                `Provided timestamp older than 10 minutes. Ignoring event ${eventId} from ${address}.`,
            );
            throw new NoContent();
        }
        ...
        const computedSignature = `${algorithm}=${createHmac('sha256', secret)
            .update(eventId + timestamp + req['rawBody'])
            .digest('hex')}`;

        if (computedSignature !== signature) {
            this.logger.warn(
                `Computed non-match message signature:\nComputed: ${computedSignature}\nExpected: ${signature}`,
            );
