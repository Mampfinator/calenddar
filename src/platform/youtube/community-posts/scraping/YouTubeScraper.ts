...
export class YouTubeScraper {
    ...
    private formatPost(
        ...
        if (backstageAttachment) {
            const {
                ...
                pollRenderer,
                videoRenderer,
                postMultiImageRenderer: multiImageRenderer,
            } = backstageAttachment;

            const type =
                imageRenderer ?? multiImageRenderer
                    ? 'image'
                    : pollRenderer
                    ? 'poll'
                    : videoRenderer
            ...
            if (!type)
                console.error(
                    `Could not find type for backstageAttachment! Details: `,
                    backstageAttachment,
                );
            ...
            switch (type) {
                case 'image':
                    const images = [
                        imageRenderer?.image,
                        ...(multiImageRenderer?.images.map(
                            (renderer) => renderer.backstageImageRenderer.image,
                        ) ?? []),
                    ].filter((v) => v);
                    attachment.images = images
                        .map((image) => last<any>(image.thumbnails).url)
                        .map((url) => `${url.split('=')[0]}=s8000`);
                    break;
                case 'poll':
                    attachment.choices = pollRenderer.choices.map((choice) =>
                        choice.text.runs.map((run) => run.text).join(''),
                    );
                    break;
                case 'video':
                    ...
                    const thumbnail = last<any>(
                        videoRenderer.thumbnail.thumbnails,
                    ).url;
                    const title =
                        videoRenderer.title.runs
                            ?.map((run) => run.text)
                            .join('') ?? videoRenderer.title.simpletext;
                    const descriptionSnippet =
                        videoRenderer.descriptionSnippet?.runs
                            ?.map((run) => run.text)
                            .join('') ??
    ...
    public async fetchPost(id: string): Promise<CommunityPost> {
        ...
        const initialData = this.parseYtInitialData(
            await axios
                .get(`https://www.youtube.com/post/${id}`, {
    ...
    public async fetchPosts(
        ...
        const initialData = this.parseYtInitialData(
            await axios
                .get(`https://www.youtube.com/channel/${channelId}/community`, {
                    transformResponse: (res) => res,
                })
                .then((res) => res.data),
        );
        ...
        const rawPosts = findValuesByKeys(postRenderers, [
            'backstagePostRenderer',
        ]);

        return new Map<string, CommunityPost>(
