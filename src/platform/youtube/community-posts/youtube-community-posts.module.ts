import { forwardRef, Logger, Module, OnApplicationBootstrap, OnApplicationShutdown } from "@nestjs/common";
import { InjectConnection, MongooseModule, SchemaFactory } from "@nestjs/mongoose";
...
@Module({
    ...
    controllers: [
        YouTubeCommunityPostsController
    ]
})
export class YouTubeCommunityPostsModule implements OnApplicationBootstrap, OnApplicationShutdown {
    ...
    constructor(
        ...
        let isReady = (id: string) => {
            ...
            if (this._initialSet.size >= this._initialThreshold) {
                this.logger.log("Finished initial fetching of community posts!");
        ...
        this._timer = new DynamicTimer(
            async () => {
                const time = (await this.vtuberRepository.getAllYouTubeIds()).size * 2500
                return time;
            },
            async () => {
                ...
                [...ids].forEach((id, index) => {
                    setTimeout(async () => {
                        isReady(id);
                        await this.postsService.syncPostsById(id).catch(error => this.logger.error(error));
                    }, index * 2250);
                })
            } 
        )
    ...
    async onApplicationBootstrap() {
        ...
        this._initialThreshold = (await this.vtuberRepository.getAllYouTubeIds()).size;
