...
export class CommunityPostCreatedHandler implements IEventHandler<CommunityPostCreatedEvent>{
    constructor(
    ...
    handle({post}: CommunityPostCreatedEvent) {
        ...
        if (this.communityPostService.hasFetchedInitial()) this.eventEmitter.emit("webevents.youtube.post", CommunityPost.fromRoot(post));
    }
}